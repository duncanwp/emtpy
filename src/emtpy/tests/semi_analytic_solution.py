...
class AnalyticSolution(object):
    ...
    def __init__(self, L, V_0, tol=1.0E-3):
        ...
        self.u_0_sq = (me * L**2 * V_0**2)/(2 * hbar**2)
    ...
    def finite_square_box_energies(self):
        ...
        initial_guess = initial_guess_at_roots(self.symmetric_solution, sqrt(self.u_0_sq)/100.0, 0.0+sqrt(self.u_0_sq)/100.0, sqrt(self.u_0_sq))

        symm_sol = brentq(self.symmetric_solution, initial_guess[0][0], initial_guess[0][1])

        solutions, info, ierr, mesg = fsolve(self.symmetric_solution_vec, initial_guess, full_output=True)
        if ierr != 1:
            print mesg
            assert False
        anti_solutions, info, ierr, mesg = fsolve(self.anti_symmetric_solution, sqrt(linspace(0.0 +  sqrt(self.u_0_sq)/5.0, sqrt(self.u_0_sq),5)), full_output=True)
        ...
        energies = [self.energy(e) for e in sols] # if e < sqrt(u_0_sq)]
        energies.sort()
        return energies